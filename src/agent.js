...
class Agent {
    ...
    /**
     * Toggle whether the adb daemon on the device will use authorization. This is a requirement
     * for the port to be exposed without using VPN.
     *
     * You must add a key for usage of ADB after this, @see Agent#addAdbKey
     *
     * This has no effect on an iOS vm.
     *
     * @example
     * let agent = await instance.newAgent();
     * await agent.ready();
     * await agent.toggleAdbAuth(true);
     * await agent.addAdbKey("QAAAAO...A= key@machine");
     */
    async toggleAdbAuth(enableAuth) {
        var enable = "false";
        if (enableAuth)
            enable = "true";

        await this.command('system', 'adb-auth', {enable});
    }

    /**
     * Add a key for usage with adb connections. This is normally the key found in
     * `~/.android/adbkey.pub` that is automatically generated by the android adb
     * tool.
     *
     * You must enable usage for key authentication in ADB before this will take
     * any effect, @see Agent#toggleAdbAuth
     *
     * This has no effect on an iOS vm.
     *
     * @example
     ...
     */
    async addAdbKey(key) {
        await this.command('system', 'adb-key', {key});
    }
