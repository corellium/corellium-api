  ...
  describe('projects', function () {
    ...
    it('lists projects', async function () {
      project = await corellium.projects().then(projects => {
        const foundProject = projects.find(project => project.info.name === CONFIGURATION.project)
        assert(
    ...
    it('can list supported devices', async function () {
      ...
      const firmware = supportedDevices.find(device => device.name === CONFIGURATION.testFlavor)
      assert(firmware)
    })
    ...
    describe(`instances ${instanceVersion}`, function () {
      ...
      describe(`snapshots ${instanceVersion}`, function () {
        ...
        it('can restore a snapshot', async function () {
          assert(latestSnapshot, 'This test cannot run because there is no latestSnapshot to use')
          const instance = instanceMap.get(instanceVersion)
          if (CONFIGURATION.testFlavor === 'ranchu') {
        ...
        it('can delete a snapshot', async function () {
          ...
          const instance = instanceMap.get(instanceVersion)
          assert((await instance.snapshots()).some(snapshot => snapshot.id === latestSnapshot.id))

          await latestSnapshot.delete()

          while ((await instance.snapshots()).some(snapshot => snapshot.id === latestSnapshot.id));
        })
      ...
      describe(`agent ${instanceVersion}`, function () {
        ...
        it('uses the correct Websocket url', async function () {
          if (/^https/.test(CONFIGURATION.endpoint)) {
            assert.match(agent.ws.url, /wss/)
          } else {
            assert.match(agent.ws.url, /ws/)
          }
        })
          ...
          it('can delete files', async function () {
            ...
            try {
              ...
              if (CONFIGURATION.testFlavor === 'ranchu') {
                ...
                assert(error.toString().includes("Stat of file '" + testPath + "' failed."))
              }
            }
          })
        })
        ...
        describe(`crash watcher ${instanceVersion}`, function () {
          ...
          after(
            setFlagIfHookFailedDecorator(async function () {
              if (crashListener !== undefined && crashListener.connected) crashListener.disconnect()
            })
          )
        ...
        describe(`Network Monitor ${instanceVersion}`, function () {
          ...
          it('uses the correct Websocket url', async function () {
            if (/^https/.test(CONFIGURATION.endpoint)) {
              assert.match(netmon.instance._agent.ws.url, /wss/)
            } else {
              assert.match(netmon.instance._agent.ws.url, /ws/)
            }
          })
          ...
          it('can monitor data', async function () {
            ...
            return new Promise(resolve => {
              ...
              return new Promise(resolve => setTimeout(resolve, 1000 * 5)).then(async () => {
                if (CONFIGURATION.testFlavor === 'ranchu') {
          ...
          describe('frida attaching and execution', function () {
            if (CONFIGURATION.testFlavor === 'ranchu') {
              it('can attach frida', async function () {
                ...
                } while (!(processList.attached && processList.attached.target_name === name))
              })
        ...
        describe(`CoreTrace ${instanceVersion}`, function () {
          ...
          it.skip('can capture data', async function () {
            let statTarget = ''
            const instance = instanceMap.get(instanceVersion)
            if (CONFIGURATION.testFlavor === 'ranchu') statTarget = '/data/corellium/frida/scripts/'
