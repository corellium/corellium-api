...
describe("Corellium API", function () {
  ...
  describe("projects", function () {
    ...
    it(`has room for ${INSTANCE_VERSIONS.length} new VMs (get quota / quotasUsed)`, async function () {
      assert(
        project,
        "Unable to test as no project was returned from previous tests"
      );
    ...
    it("can list supported devices", async function () {
      ...
      const firmware = supportedDevices.find(
        (device) => device.name === config.testFlavor
      );
      assert(firmware);
    });
    ...
    it("can get roles", async function () {
      ...
      assert(
        roles,
        "Roles should not be undefined, even if there have been no roles"
      );
    });
  ...
  INSTANCE_VERSIONS.forEach((instanceVersion) => {
    describe(`panics ${instanceVersion}`, function () {
      ...
      it("can request panics", async function () {
        ...
        assert(
          panics,
          "Panics should not be undefined, even if there have been no panics"
        );
      });
    ...
    describe(`instances ${instanceVersion}`, function () {
      ...
      it("can take a screenshot", async function () {
        ...
        await instance.takeScreenshot().then((png) => {
          ...
          assert(
            png.compare(expected, 0, expected.length, 0, expected.length) === 0
          );
        });
      });
          ...
          it("can install a signed apk", async function () {
            ...
            while (true) {
              ...
              let rs = fs.createReadStream(
                path.join(__dirname, "api-test.apk")
              );
              try {
        ...
        describe(`Frida ${instanceVersion}`, function () {
          ...
          describe("frida attaching and execution", async function () {
            ...
            it.skip("can execute script", async function () {
              ...
              assert(
                fridaOutput.toString().includes("Hook android_log_write()")
              );
            });
